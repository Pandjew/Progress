...
function Grid({ cols = 2, gap = 14, children, style, mobileCols }) {
  const mc = mobileCols !== undefined ? mobileCols : (cols > 2 ? 2 : 1);
...
export default function App() {
  ...
  return <div style={{ minHeight: "100vh", background: C.bg, color: C.text, fontFamily: font, paddingBottom: 70 }}>
    ...
    <style>{`
      ...
      .tp-grid-5 { grid-template-columns: repeat(5, 1fr); }
      @media (max-width: 700px) {
        .tp-grid-m1 { grid-template-columns: 1fr !important; }
        .tp-grid-m2 { grid-template-columns: repeat(2, 1fr) !important; }
        .tp-grid-m3 { grid-template-columns: repeat(3, 1fr) !important; }
        .tp-nav-top { display: none !important; }
        .tp-nav-bottom { display: flex !important; }
        .tp-header-title { font-size: 15px !important; }
        .tp-stat-row { flex-wrap: wrap; }
        .tp-stat-row > div { min-width: 45% !important; flex: 1 1 45% !important; }
        .tp-badge-row { flex-wrap: wrap; gap: 3px !important; }
      ...
      @media (min-width: 701px) {
        ...
        .tp-nav-top { display: flex !important; }
      }
      /* Safe area for iPhone notch */
      @supports (padding-top: env(safe-area-inset-top)) {
        .tp-header { padding-top: calc(14px + env(safe-area-inset-top)); }
        .tp-bottom-nav { padding-bottom: calc(6px + env(safe-area-inset-bottom)); }
      }
    `}</style>
    ...
    <div className="tp-nav-bottom tp-bottom-nav" style={{
      display: "none", position: "fixed", bottom: 0, left: 0, right: 0,
      background: C.card, borderTop: `1px solid ${C.border}`,
      padding: "6px 4px", zIndex: 100, justifyContent: "space-around",
      backdropFilter: "blur(20px)", WebkitBackdropFilter: "blur(20px)",
    }}>
      {tabs.map(t => <button key={t.id} onClick={() => setTab(t.id)} style={{
        background: "none", border: "none", display: "flex", flexDirection: "column",
        alignItems: "center", gap: 2, padding: "4px 6px", cursor: "pointer",
        color: tab === t.id ? C.accent : C.textMuted, fontSize: 9, fontWeight: tab === t.id ? 700 : 500,
        fontFamily: font, minWidth: 0, transition: "color .15s",
      }}>
...
function DashboardView({ workouts, nutrition, body, shoes, shoeKm }) {
  ...
  return <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
    <div className="tp-stat-row" style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
    ...
    <Grid cols={2} gap={16} mobileCols={1}>
    ...
    <Card title="5 dernières séances">
      <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
        {workouts.slice(0, 5).map(w => <div key={w.id} className="tp-history-item" style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "8px 12px", background: C.bg, borderRadius: 8 }}>
          ...
          <div className="tp-badge-row tp-history-badges" style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
    ...
    <Card title="🥗 Saisie nutrition">
      ...
      {mode === "macros" ? <>
        ...
        {meals.map((ml, i) => <div key={i} style={{ padding: 12, background: C.bg, borderRadius: 10 }}>
          ...
          <Grid cols={4} gap={8} mobileCols={2}>
